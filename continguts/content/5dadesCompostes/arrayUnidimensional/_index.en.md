...
{{% notice note %}}
**Arrays unidimensional**: és una estructura de dades que conté una col·lecció de dades del mateix tipus. 
{{% /notice %}}
...
[Java Oracle Classe Arrays](https://docs.oracle.com/javase/8/docs/api/java/util/Arrays.html)
...
- Als elements de l'array si accedirà a través de la posició que ocupa dins el conjunt d'elements de l'array.
...
És important entendre que un cop declaro un arrayEstatic, es reserva en memòria espai per guardar el ``tipus de dades * Tamany de l'array``. En l'exemple anterior es demana al programa que ens reservi espai per a 12 enters consecutius. Aquests enters no tenen un valor definit (al ser enters Java els hi posa un 0, si fossin booleans el valor per defecte seria false), per tant cal que els inicialitzem un valor per utilitzar-los.
...
#### Longitud
Al ser una estructura estàtica la **longitud** de l'array no es pot variar i es defineix quan es crea (amb el new). Per tal d'obtenir el seu valor en Java utilitzarem la funció ``length``:
```java
...
#### Ordenació arrays
Una altra funcionalitat corrent és l'ordenació de arrays. En Java existeix una funció que ordena els elements d'un array, més endavant l'estudiarem amb deteniment.
```java
Arrays.sort(vector);
```
...
Per tal d'efectuar el recorregut en un array java ens permet aquesta expressió reduïda de bucles for. Per exemple, els bucles següents són equivalents:
...
```java
int[] llista = {1,2,3,4};
//llita i llistaCopia apunten al mateix array ja que són referències
...
while(inferior<=superior && !trobat){
    ...
    System.out.println("Iteracio: " + iteracions + "  Inferior:" + inferior + "  Superior:" + superior + "  Element Central:" + vector[elementCentral] + "  A buscar:" + nombreBuscat);
...
while(inferior<=superior && !trobat) {
  ...
  //Imprimim l'estat actual
  System.out.println("Iteracio: " + iteracions + "  Inferior:" + inferior + "  Superior:" + superior + "  Element Actual:" + vector[inferior] + "  A buscar:" + nombreBuscat);
